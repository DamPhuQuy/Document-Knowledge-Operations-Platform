...
<foundations>
  ...
  <pillar id="context" title="Context Navigation">
    ...
    <information_priority>
      ...
      2. TypeScript Interfaces & Types (`src/types/`, `src/models/`)
      3. API Service Layer & Custom Hooks (`src/services/`, `src/hooks/`, `src/api/`)
      4. Configuration & Tooling (`vite.config.ts`, `package.json`, `tsconfig.json`)
      5. UI Components & Views (`src/components/`, `src/pages/`, `src/views/`)
    </information_priority>
  </pillar>
  ...
  <pillar id="harness" title="Engineering Harness & Guardrails">
    ...
    <architecture_guardrail>
      React 19 + TypeScript + Vite. Component-driven architecture with clean separation of UI presentation, custom hooks for state/business logic, and API clients. Maintain strict TypeScript types (never use `any` or disable ESLint rules without authorization).
    </architecture_guardrail>
  </pillar>
</foundations>
