...
export const AuthScreen: React.FC<AuthScreenProps> = ({ onSuccess }) => {
  ...
  const handleRegister = async (values: RegisterRequest) => {
    try {
      ...
      otpForm.resetFields();
      message.success(
        "Account registered successfully. Please enter the OTP verification code to activate your account.",
      );
    } catch {
      // Handled by context error
    }
  ...
  const handleVerifyOtp = async (values: { otp: string }) => {
    try {
      ...
      message.success(
        "Identity verified and account activated. Please sign in to continue.",
      );
      setOtpStep(false);
      setActiveTab("signin");
      loginForm.setFieldsValue({ email: pendingEmail });
    } catch {
      // Handled by context error
    }
  };
      ...
      >
        <div style={{ marginBottom: 20 }}>
          <h2 style={{ fontSize: 22, fontWeight: 700, color: "#0f172a" }}>
            {isSignIn ? "Sign In to DocOps" : "Create Account"}
          </h2>
    ...
    <div className="midone-auth-wrapper">
      ...
      <div className="midone-auth-brand-side">
        ...
        <div style={{ textAlign: "center", margin: "auto 0" }}>
          ...
          >
            Multi-tenant cryptographic document storage, knowledge management,
            and operational platform for secure document handling and
            collaboration.
          </p>
