...
class DepartmentControllerTest {
    ...
    void setUp() {
        ...
        DepartmentJpaEntity hr = DepartmentJpaEntity.builder()
            .id(hrDeptId)
            .code("HR")
        ...
        DepartmentJpaEntity it = DepartmentJpaEntity.builder()
            .id(itDeptId)
            .code("IT")
    ...
    @WithMockUser(roles = "ADMIN")
    @DisplayName(
        "POST /api/v1/departments - 400 Bad Request on invalid uppercase alphanumeric code (Rule B2)"
    )
    ...
    @WithMockUser(roles = "ADMIN")
    @DisplayName(
        "POST /api/v1/departments - 409 Conflict on duplicate department code (Path 2a)"
    )
    ...
    @WithMockUser(roles = "ADMIN")
    @DisplayName(
        "PUT /api/v1/departments/{id} - 409 Conflict when updating to another existing code"
    )
    void updateDepartment_DuplicateCode() throws Exception {
        UpdateDepartmentRequest request = new UpdateDepartmentRequest(
            "IT",
            "Try renaming HR to IT",
            "Conflict"
        );
    ...
    @WithMockUser(roles = "ADMIN")
    @DisplayName(
        "PUT /api/v1/departments/{id} - 404 Not Found on non-existent department"
    )
    ...
    @WithMockUser(roles = "ADMIN")
    @DisplayName(
        "GET /api/v1/departments/{id} - 404 Not Found on missing department"
    )
