...
class UserRoleControllerTest {
  ...
  @BeforeEach
  void setUp() {
    PermissionJpaEntity manageUsersPerm =
  ...
  @DisplayName("Should return 403 when user lacks manage:users and ROLE_ADMIN")
  ...
  void shouldAssignRolesSuccessfully() throws Exception {
    AssignRolesRequest request = new AssignRolesRequest(Set.of(staffRoleId, managerRoleId));
  ...
  @Test
  @DisplayName("Should reject admin revoking own ROLE_ADMIN with 400 Bad Request")
  void shouldRejectAdminSelfRoleRevocation() throws Exception {
    AssignRolesRequest request = new AssignRolesRequest(Set.of(staffRoleId));

    mockMvc
        .perform(
            put("/api/v1/users/{userId}/roles", adminUserId)
                .with(org.springframework.security.test.web.servlet.request.SecurityMockMvcRequestPostProcessors.user(adminUserId.toString()).roles("ADMIN").authorities(new org.springframework.security.core.authority.SimpleGrantedAuthority("manage:users"), new org.springframework.security.core.authority.SimpleGrantedAuthority("ROLE_ADMIN")))
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.message", org.hamcrest.Matchers.containsString("cannot revoke their own ROLE_ADMIN")));
  }
}
