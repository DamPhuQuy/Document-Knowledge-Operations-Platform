...
@Slf4j
public class AuditLoggingService implements RecordAuditLogUseCase, GetAuditLogsUseCase {
  ...
  public AuditLog recordAuditLog(RecordAuditLogCommand command) {
    Objects.requireNonNull(command, "RecordAuditLogCommand must not be null");
    ...
    AuditLog auditLog =
        AuditLog.builder()
            ...
            .status(command.getStatus() != null ? command.getStatus() : AuditStatus.SUCCESS)
  ...
  public Page<AuditLogResponseDto> getAuditLogs(AuditLogQueryFilter filter, Pageable pageable) {
