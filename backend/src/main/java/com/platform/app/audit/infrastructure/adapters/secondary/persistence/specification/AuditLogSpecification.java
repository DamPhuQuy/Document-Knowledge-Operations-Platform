...
public final class AuditLogSpecification {
    ...
    public static Specification<AuditLogJpaEntity> withFilter(
        ...
        return (root, query, cb) -> {
            ...
            if (filter.getAction() != null && !filter.getAction().isBlank()) {
                predicates.add(
                    cb.equal(
                        ...
                        filter.getAction().trim().toUpperCase()
                    )
                );
            }
            ...
            if (
                ...
                !filter.getResourceType().isBlank()
            ) {
                predicates.add(
                    cb.equal(
            ...
            if (
                ...
                !filter.getResourceId().isBlank()
            ) {
                predicates.add(
                    cb.equal(
                        root.get("resourceId"),
                        filter.getResourceId().trim()
                    )
                );
            }
