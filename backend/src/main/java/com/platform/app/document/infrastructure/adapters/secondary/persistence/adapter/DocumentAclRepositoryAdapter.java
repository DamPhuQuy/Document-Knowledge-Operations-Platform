...
public class DocumentAclRepositoryAdapter implements DocumentAclRepositoryPort {
    ...
    public void replacePermissions(
        ...
        if (userGrants != null && !userGrants.isEmpty()) {
            List<DocumentUserAccessJpaEntity> userEntities = userGrants
                .stream()
                .map(this::toEntity)
                .toList();
        ...
        if (departmentGrants != null && !departmentGrants.isEmpty()) {
            List<DocumentDepartmentAccessJpaEntity> deptEntities =
                departmentGrants.stream().map(this::toEntity).toList();
            departmentAccessRepository.saveAll(deptEntities);
        }

        // 4. Insert new role grants
        if (roleGrants != null && !roleGrants.isEmpty()) {
            List<DocumentRoleAccessJpaEntity> roleEntities = roleGrants
                .stream()
                .map(this::toEntity)
                .toList();
        ...
        log.debug(
            "Replaced permissions for documentId={}: {} users, {} depts, {} roles",
            documentId,
    ...
    @Override
    public List<DocumentDepartmentAccess> findDepartmentGrants(
        UUID documentId
    ) {
        Objects.requireNonNull(documentId, "Document ID must not be null");
        return departmentAccessRepository
            .findByDocumentId(documentId)
            .stream()
            .map(this::toDomain)
            .toList();
    ...
    public boolean hasUserPermission(
        ...
        List<DocumentUserAccessJpaEntity> grants =
            userAccessRepository.findByDocumentId(documentId);
        return grants
            .stream()
            .filter(g -> g.getUserId().equals(userId))
            .anyMatch(g ->
                isPermissionSatisfied(g.getPermissionLevel(), requiredLevel)
            );
    }

    private boolean isPermissionSatisfied(
    ...
    public DocumentDepartmentAccessJpaEntity toEntity(
        DocumentDepartmentAccess domain
    ) {
        if (domain == null) {
            return null;
        }
    ...
    public DocumentDepartmentAccess toDomain(
        DocumentDepartmentAccessJpaEntity entity
    ) {
        if (entity == null) {
            return null;
        }
