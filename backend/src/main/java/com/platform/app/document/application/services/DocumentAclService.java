...
public class DocumentAclService
    implements ConfigureDocumentAclUseCase, GetDocumentPermissionsUseCase
{
    ...
    @Override
    @Transactional
    public DocumentPermissionsResponseDto configureAcl(
        ConfigureDocumentAclCommand command
    ) {
        Objects.requireNonNull(
            command,
            "Configure ACL command must not be null"
        );
        Objects.requireNonNull(
        ...
        boolean isAuthorized =
            isOwner || command.isAdmin() || command.isHasManagePermissions();

        if (!isAuthorized) {
            log.warn(
                "Access denied for user {} attempting to configure ACL on document {} owned by {}",
                command.getCurrentUserId(),
                document.getId(),
                document.getUploadedByUserId()
            );
            throw new DocumentAccessDeniedException(
                "User does not have permission to configure ACL for document " +
                    command.getDocumentId()
            );
        }

        List<DocumentUserAccess> userGrants =
            command.getUserGrants() != null
                ? command
                      .getUserGrants()
                      .stream()
                      .map(g ->
        ...
        List<DocumentDepartmentAccess> deptGrants =
            command.getDepartmentGrants() != null
                ? command
                      .getDepartmentGrants()
                      .stream()
                      .map(g ->
        ...
        documentAclRepositoryPort.replacePermissions(
            savedDoc.getId(),
            userGrants,
            deptGrants,
            roleGrants
        ...
        log.info(
            "Successfully updated ACL for document {}: level={}, users={}, depts={}, roles={}",
            savedDoc.getId(),
            savedDoc.getAccessLevel(),
            userGrants.size(),
            deptGrants.size(),
            roleGrants.size()
        );
    ...
    public DocumentPermissionsResponseDto getPermissions(
        ...
        boolean isOwner =
            currentUserId != null &&
            document.getUploadedByUserId().equals(currentUserId);
        boolean isPrivileged = isAdmin || hasManagePermissions;

        if (
            document.getAccessLevel() == AccessLevel.CONFIDENTIAL &&
            !isOwner &&
            !isPrivileged
        ) {
            boolean hasUserGrant =
                currentUserId != null &&
                documentAclRepositoryPort.hasUserPermission(
                    documentId,
                    currentUserId,
            ...
            if (!hasUserGrant) {
                log.warn(
                    "Access denied for user {} attempting to view ACL on confidential document {}",
                    currentUserId,
                    documentId
                );
                throw new DocumentAccessDeniedException(
                    "User does not have permission to view permissions for document " +
                        documentId
                );
            }
        }
        ...
        List<DocumentUserAccess> userGrants =
            documentAclRepositoryPort.findUserGrants(documentId);
        List<DocumentDepartmentAccess> deptGrants =
            documentAclRepositoryPort.findDepartmentGrants(documentId);
        List<DocumentRoleAccess> roleGrants =
            documentAclRepositoryPort.findRoleGrants(documentId);
    ...
    private List<UserGrantDto> toUserGrantDtos(
        List<DocumentUserAccess> grants
    ) {
        if (grants == null) return Collections.emptyList();
        return grants
            .stream()
            .map(g ->
                UserGrantDto.builder()
                    .userId(g.getUserId())
                    .permissionLevel(g.getPermissionLevel())
                    .build()
            )
    ...
    private List<DepartmentGrantDto> toDeptGrantDtos(
        List<DocumentDepartmentAccess> grants
    ) {
        if (grants == null) return Collections.emptyList();
        return grants
            .stream()
            .map(g ->
                DepartmentGrantDto.builder()
                    .departmentId(g.getDepartmentId())
                    .permissionLevel(g.getPermissionLevel())
                    .build()
            )
    ...
    private List<RoleGrantDto> toRoleGrantDtos(
        List<DocumentRoleAccess> grants
    ) {
        if (grants == null) return Collections.emptyList();
        return grants
            .stream()
            .map(g ->
                RoleGrantDto.builder()
                    .roleId(g.getRoleId())
                    .permissionLevel(g.getPermissionLevel())
                    .build()
            )
