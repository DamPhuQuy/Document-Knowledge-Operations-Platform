...
public class DocumentVersionService implements UploadDocumentVersionUseCase {
    ...
    @Override
    public DocumentVersionResponseDto uploadVersion(
        UploadDocumentVersionCommand command
    ) {
        Objects.requireNonNull(
            command,
            "Upload version command must not be null"
        );
        Objects.requireNonNull(
        ...
        if (originalFileName == null || originalFileName.trim().isEmpty()) {
            throw new DocumentValidationException(
                "Original file name must not be blank"
            );
        }
        ...
        if (
            extension == null ||
            !ALLOWED_EXTENSIONS.contains(extension.toUpperCase())
        ) {
        ...
        if (command.getFileSize() > Document.MAX_FILE_SIZE_BYTES) {
            throw new PayloadTooLargeException(
                "File size exceeds maximum allowed 50MB limit"
            );
        }
        ...
        if (
            !document.getUploadedByUserId().equals(command.getUserId()) &&
            !command.isAdmin()
        ) {
            log.warn(
                "Access denied for user {} attempting to upload new version for document {} owned by {}",
                command.getUserId(),
                document.getId(),
                document.getUploadedByUserId()
            );
            throw new DocumentAccessDeniedException(
                "User does not have permission to upload a new version for document " +
                    command.getDocumentId()
            );
        }
        ...
        try {
            ...
            throw new IllegalStateException(
                "SHA-256 algorithm not available in JVM",
                e
            );
        }
        ...
        log.info(
            "Starting streaming S3 upload for documentId={}, version={}, key={}, size={}",
            document.getId(),
            nextVersion,
            storageKey,
            command.getFileSize()
        ...
        log.info(
            "Computed SHA-256 for documentId={}, version={}: {}",
            document.getId(),
            nextVersion,
            checksumSha256
        );
        ...
        try {
            ...
            log.error(
                "Failed to persist document version metadata for docId={}, version={}. Executing S3 compensation delete for key: {}",
                document.getId(),
                nextVersion,
                storageKey,
                e
            ...
            try {
                ...
                log.error(
                    "S3 compensation delete failed for key: {}",
                    storageKey,
                    delEx
                );
            }
