...
public class DocumentUploadService implements UploadDocumentUseCase {
    ...
    public DocumentResponseDto uploadDocument(UploadDocumentCommand command) {
        ...
        log.info(
            "Starting streaming S3 upload for documentId={}, key={}, size={}",
            docId,
            storageKey,
            command.getFileSize()
        );
        ...
        log.info(
            "Computed SHA-256 for documentId={}: {}",
            docId,
            checksumSha256
        );
        ...
        try {
            savedDocument = storeMetadataUseCase.persistMetadata(
                docId,
                command,
                sanitizedFileName,
                storageKey,
            ...
            log.error(
                "Failed to persist document metadata for docId={}. Executing S3 compensation delete for key: {}",
                docId,
                storageKey,
                e
            );
