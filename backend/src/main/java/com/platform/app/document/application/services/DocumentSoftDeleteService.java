...
public class DocumentSoftDeleteService implements SoftDeleteDocumentUseCase {
  ...
  public void softDeleteDocument(SoftDeleteDocumentCommand command) {
    ...
    boolean isAuthorized = isOwner || command.isAdmin() || command.isHasDeletePermission();

    if (!isAuthorized) {
      log.warn("Access denied for user {} attempting to soft-delete document {} owned by {}",
          command.getCurrentUserId(), document.getId(), document.getUploadedByUserId());
      throw new DocumentAccessDeniedException("User does not have permission to delete document " + command.getDocumentId());
    }

    Instant deletedAt = Instant.now();
    boolean updated = documentRepositoryPort.softDelete(document.getId(), deletedAt);
    if (!updated) {
      log.warn("Document {} was concurrently modified or already deleted", document.getId());
      throw new DocumentNotFoundException("Document not found or already deleted: " + command.getDocumentId());
    }
    ...
    log.info("Successfully soft-deleted document {} by user {}", document.getId(), command.getCurrentUserId());
  }
}
