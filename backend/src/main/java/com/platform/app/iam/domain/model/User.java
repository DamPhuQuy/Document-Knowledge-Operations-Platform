...
public class User {
    ...
    public User(
        ...
        this.passwordHash = Objects.requireNonNull(
            passwordHash,
            "User passwordHash must not be null"
        );
        this.fullName = Objects.requireNonNull(
            fullName,
            "User fullName must not be null"
        );
    ...
    public void assignRoles(Set<Role> newRoles, UUID operatorUserId) {
        ...
        boolean isSelf =
            operatorUserId != null && this.id.equals(operatorUserId);
        boolean hasAdminRole = newRoles
            .stream()
            .anyMatch(
                r ->
        ...
        boolean currentlyAdmin = this.roles
            .stream()
            .anyMatch(
                r ->
                    "ROLE_ADMIN".equalsIgnoreCase(r.getCode()) ||
