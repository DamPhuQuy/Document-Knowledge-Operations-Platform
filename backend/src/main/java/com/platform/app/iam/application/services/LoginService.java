...
public class LoginService implements LoginUseCase {
    ...
    public AuthTokensDto execute(LoginCommand command) {
        ...
        if (accountLockoutPort.isLocked(email)) {
            log.warn(
                "Login attempt blocked for email [{}]: account locked",
                email
            );
        ...
        if (userOptional.isEmpty()) {
            ...
            log.warn(
                "Login attempt failed for email [{}]: user not found",
                email
            );
            eventPublisher.publishEvent(
                UserLoginFailedEvent.builder()
        ...
        if (!user.isEnabled()) {
            log.warn(
                "Login attempt rejected for email [{}]: account deactivated",
                email
            );
            eventPublisher.publishEvent(
                UserLoginFailedEvent.builder()
        ...
        if (
            ...
            accountLockoutPort.recordFailure(email);
            log.warn(
                "Login attempt failed for email [{}]: invalid password",
                email
            );
            eventPublisher.publishEvent(
                UserLoginFailedEvent.builder()
        ...
        log.info(
            "User [{}] authenticated successfully with {} roles",
            user.getId(),
            user.getRoleCodes().size()
        );

        // 6. Generate access token & refresh token
        String accessToken = tokenProviderPort.generateAccessToken(user);
        String refreshTokenString =
            tokenProviderPort.generateRefreshTokenString();
