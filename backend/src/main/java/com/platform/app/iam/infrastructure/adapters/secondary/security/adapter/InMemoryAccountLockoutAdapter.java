...
public class InMemoryAccountLockoutAdapter implements AccountLockoutPort {
    ...
    public boolean isLocked(String email) {
        ...
        if (record.lockedUntil() != null) {
            if (now.isBefore(record.lockedUntil())) {
                log.warn(
                    "Account [{}] is currently locked until {}",
                    normalized,
                    record.lockedUntil()
                );
                return true;
            ...
            log.info(
                "Lockout expired for account [{}]; reset to unlocked",
                normalized
            );
            return false;
        }
    ...
    public void recordFailure(String email) {
        ...
        attemptCache.compute(normalized, (key, current) -> {
            if (current == null) {
            ...
            if (
                Duration.between(current.firstFailedAt(), now).compareTo(
                    WINDOW_DURATION
                ) > 0
            ) {
                return new AttemptRecord(1, now, null);
