...
public class AuthController {
    ...
    @Operation(
        summary = "User Login",
        description = "Authenticates user credentials and issues a JWT Access Token and long-lived Refresh Token"
    )
    @ApiResponse(
        responseCode = "200",
    ...
    public ResponseEntity<LoginResponse> login(
        ...
        log.info(
            "REST POST /api/v1/auth/login received for email [{}] from IP [{}]",
            request.email(),
            clientIp
        );
        ...
        log.debug(
            "REST POST /api/v1/auth/login succeeded for user [{}]",
    ...
    public ResponseEntity<RegisterResponse> register(
        ...
        log.info(
            "REST POST /api/v1/auth/register received for email [{}]",
            request.email()
        );
        ...
        log.debug(
            "REST POST /api/v1/auth/register succeeded for user [{}]",
            profile.id()
        );

        return ResponseEntity.status(HttpStatus.CREATED).body(
            RegisterResponse.from(profile)
        );
    }
    ...
    public ResponseEntity<VerifyOtpResponse> verifyOtp(
        ...
        log.info(
            "REST POST /api/v1/auth/verify-otp received for email [{}]",
            request.email()
        );
        ...
        log.debug(
            "REST POST /api/v1/auth/verify-otp succeeded for email [{}]",
            request.email()
        );
