...
public class UserRoleController {
    ...
    @GetMapping()
    @PreAuthorize(
        "hasAuthority('manage:users') or hasAuthority('MANAGE:USERS') or hasRole('ADMIN')"
    )
    public ResponseEntity<UserRolesResponseDto> getUserRoles(
        @PathVariable("userId") UUID userId
    ) {
        log.debug("REST GET /api/v1/users/{}/roles requested", userId);
        UserRolesResponseDto response = getUserRolesUseCase.getUserRoles(
            userId
        );
    ...
    public ResponseEntity<UserRolesResponseDto> assignRoles(
        ...
        log.debug(
            "REST PUT /api/v1/users/{}/roles completed successfully for user [{}]",
            userId,
            response.userId()
        );
        return ResponseEntity.ok(response);
