...
public class DepartmentController {
  ...
  public ResponseEntity<DepartmentResponseDto> createDepartment(
    ...
    log.info("REST POST /api/v1/departments requested with code [{}] by operator [{}]", request.code(), operatorUserId);
    CreateDepartmentCommand command =
        new CreateDepartmentCommand(
            request.code(), request.name(), request.description(), operatorUserId);

    DepartmentResponseDto created = createDepartmentUseCase.createDepartment(command);
    log.debug("REST POST /api/v1/departments created department [{}] with id [{}]", created.code(), created.id());
  ...
  public ResponseEntity<DepartmentResponseDto> updateDepartment(
    ...
    UUID operatorUserId = extractOperatorId(authentication);
    log.info("REST PUT /api/v1/departments/{} requested with code [{}] by operator [{}]", id, request.code(), operatorUserId);
    UpdateDepartmentCommand command =
        new UpdateDepartmentCommand(
            id, request.code(), request.name(), request.description(), operatorUserId);
  ...
  public ResponseEntity<List<DepartmentResponseDto>> listDepartments() {
    ...
    List<DepartmentResponseDto> departments = getDepartmentUseCase.listDepartments();
