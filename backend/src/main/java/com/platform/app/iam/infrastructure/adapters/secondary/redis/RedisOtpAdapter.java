...
public class RedisOtpAdapter implements OtpRepositoryPort {
  ...
  private final ConcurrentMap<String, OtpEntry> inMemoryFallback = new ConcurrentHashMap<>();
  ...
  public void saveOtp(String email, String otp, Duration ttl) {
    ...
    try {
      ...
      log.debug("Saved OTP to Redis for key [{}] with TTL [{}]", key, ttl);
    } catch (Exception ex) {
      log.warn("Redis unavailable, falling back to in-memory store for OTP key [{}]: {}", key, ex.getMessage());
      inMemoryFallback.put(key, new OtpEntry(otp, Instant.now().plus(ttl)));
    }
  }
  ...
  public Optional<String> getOtp(String email) {
    ...
    try {
      ...
      log.warn("Redis unavailable, reading OTP from in-memory fallback for key [{}]: {}", key, ex.getMessage());
    }
  ...
  public void deleteOtp(String email) {
    ...
    try {
      ...
      log.warn("Redis unavailable, deleting OTP from in-memory fallback for key [{}]: {}", key, ex.getMessage());
    }
    inMemoryFallback.remove(key);
  }
